...
(function() {
  angular.module("sample").controller("AttachementInstanceController", attachementInstanceController);

  attachementInstanceController.$inject = ['$rootScope', '$routeParams', '$location', 'AttachementService', '$window', 'CrudService', 'MessageHandlerService',
    'EntityService', '$translate', 'BreadCrumbService'];

  function attachementInstanceController($rootScope, $routeParams, $location, AttachementService, $window,
    ...
    vm.edit = edit;
    vm.save = save;
    vm.delete = deleteAttachementInstance;
    ...
    function view(id) {
      ...
      EntityService.setServicePath("admin/attachementInstance");
      EntityService.view($routeParams.id)
        .then(function (data) {
          vm.attachementInstance = data;


          /* Fill breadcrumb */
          var addOnString = vm.attachementInstance.name;
          var view = {id: "button.view", url: "#/pages/admin/attachementInstance/detail_attachementInstance/" + vm.attachementInstance.id};
    ...
    function create(attachementInstance) {

      formatEntity(attachementInstance);
	  EntityService.setServicePath("admin/attachementInstance");
      EntityService.create(attachementInstance)
        .then(function (data) {
            vm.attachementInstance = data;
            formatEntity(vm.attachementInstance);
            CrudService.setMode(CrudService.createMode());

            /* Fill breadcrumb */
            var addOnString = vm.attachementInstance.id;
            var view = {id: "button.view", url: "#/pages/admin/attachementInstance/detail_attachementInstance/" + vm.attachementInstance.id};
            BreadCrumbService.push(view);
            BreadCrumbService.appendAddOn(addOnString);
            $rootScope.breadcrumb = BreadCrumbService.getBreadCrumbStack();
    ...
    function addAnother() {
      ...
      /* Fill breadcrumb */
      var add = {id: "button.add", url: "#/pages/admin/attachementInstance/add_attachementInstance"};
      BreadCrumbService.pop();
      BreadCrumbService.push(add);
      BreadCrumbService.appendAddOn("AttachementInstance");
      $rootScope.breadcrumb = BreadCrumbService.getBreadCrumbStack();
    ...
    function edit(id) {
      ...
      EntityService.edit(id)
        .then(function (data) {
            ...
            /* Fill breadcrumb */
            var addOnString = vm.attachementInstance.id;
            var edit = {id: "button.edit", url: "#/pages/admin/attachementInstance/edit_attachementInstance/" + vm.attachementInstance.id};
            BreadCrumbService.pop();
    ...
    function save(attachementInstance) {

      formatEntity(attachementInstance);

      EntityService.setServicePath("admin/attachementInstance");
      EntityService.save(attachementInstance)
        .then(function (data) {
            vm.attachementInstance = data;
            formatEntity(vm.attachementInstance);
    ...
    function deleteAttachementInstance(id) {
      ...
      EntityService.delete(id)
        .then(function (status) {
            ...
            $location.path("#/pages/admin/attachementInstance/recherche_attachementInstance");
          }
          , function (reason) {
            if (reason == "OK")
              MessageHandlerService.displayErrorMessage(sessionExpiredMsg);
    ...
    function searchPageChanged(currentPage) {
        ...
        vm.search(vm.attachementInstance);
    };

    /*
     * Search function with criteria
     */
    function search(attachementInstance) {

      vm.searchOK = false;

      EntityService.setServicePath("admin/attachementInstance");
      EntityService.search(vm.currentPage, vm.pageSize, attachementInstance)
        .then(function (data) {
            vm.attachementInstances = data.listItems;
            vm.count = data.count;
    ...
    function toString() {
      if (angular.isDefined(vm.attachementInstance))
        return vm.attachementInstance.id;
    ...
    function init() {
        vm.attachementInstance = new Object();
    };
    ...
    function activate () {
      ...
      if (typeof($routeParams.id) != 'undefined') {
        if ($rootScope.nextUrl.endsWith("detail_attachementInstance/" + $routeParams.id))
          vm.view($routeParams.id);
        if ($rootScope.nextUrl.endsWith("edit_attachementInstance/" + $routeParams.id))
          vm.edit($routeParams.id);
        if ($rootScope.nextUrl.endsWith("delete_attachementInstance/" + $routeParams.id))
